...
### 2026-06-20 · 审计缺陷修复批次
- 完成内容：按 `docs/BUG_REPORT.md` 修复 BUG-001..BUG-017。覆盖 Auth.js credentials 直连限流、上传真实图片格式识别、消费/书影导入资源边界、`OUTBOUND_PROXY` 默认 fail closed、严格日期解析、消费 ISO 周年、账单导入同文件去重与 skipped 统计、Decimal(12,2) 应用层上限、旅行地点坐标校验与行级 `TripLocation` 存储、文章浏览量可信客户端标识与去抖写入时机、链接/消费分类排序完整集合校验。
- 关键文件：`src/lib/auth/*` 与 `src/auth.ts` 下沉登录限流；`src/lib/storage.ts`、`src/modules/*/import-limits.ts`、`src/modules/*/import-parser.ts` 收紧上传/导入入口；`src/lib/http.ts`、`src/lib/ssrf.ts` 加固出站代理；`src/lib/dayjs.ts`、todos/trips/media 相关 utils/actions 接入严格日期；`src/modules/expenses/*` 修复周报、导入幂等、金额上限与分类排序；`prisma/schema.prisma` 与 `prisma/migrations/20260620010500_trip_locations_table`、`20260620013000_harden_trip_location_migration`、`20260620014500_dedupe_trimmed_trip_locations` 实体化旅行地点并补迁移防护；`src/modules/posts/view.ts` 与 `src/app/api/posts/view/route.ts` 修复浏览量统计；`src/modules/links/*` 修复链接排序。
- 关键决定与偏离：未新增依赖，`xlsx@0.18.5` 暂不升级，改在上传与解析层限制文件大小、行数和列数以降低实际暴露面；`OUTBOUND_PROXY` 仍保留，但除非显式设置 `OUTBOUND_PROXY_TRUSTS_PRIVATE_GUARD=1`，否则默认拒绝，要求代理侧承担私网/云元数据拦截；文章浏览量默认不信任 `X-Forwarded-For`，只有在可信反向代理后显式设置 `TRUST_PROXY_HEADERS=1` 才读取代理头；旅行地点保留旧 JSON 字段一版兼容读取，但新增写入改为 `TripLocation` 行，避免 JSON 整列覆盖；消费分类排序 helper 从 `"use server"` action 文件移到 `utils.ts`，满足 Server Action 导出纪律。
- 遗留 TODO：生产如果启用 `OUTBOUND_PROXY`，必须确认代理本身拒绝 localhost、内网、链路本地与云元数据网段后再设置 `OUTBOUND_PROXY_TRUSTS_PRIVATE_GUARD=1`；生产如果启用 `TRUST_PROXY_HEADERS=1`，必须由可信反向代理清洗并覆盖外部传入的 `X-Forwarded-For` / `X-Real-IP`；`xlsx` 依赖替换仍可作为后续独立任务；当前 Codex 环境下 Windows 到 WSL Docker PostgreSQL 的 5432 端口不可达，浏览器 e2e 需用户在自己的 PowerShell 中用 `npm.cmd run dev:local` 或修复端口转发后复验。
- 验证：Task 10 局部回归 `npx.cmd vitest run src/modules/links/links.test.ts src/modules/links/actions.test.ts src/modules/expenses/actions.test.ts src/modules/expenses/expenses.test.ts src/lib/action-auth.test.ts src/lib/server-action-exports.test.ts` 通过（6 个测试文件、37 条测试）；`npx.cmd tsc --noEmit` 通过；`npm.cmd run lint` 通过；`npx.cmd vitest run` 通过（55 个测试文件、333 条测试）；`npm.cmd run check` 通过（tsc + lint + 55 个测试文件、333 条测试）；`npx.cmd prisma validate` 通过；Windows 侧 `npx.cmd prisma migrate status` 因本机 schema engine / PostgreSQL 端口链路报错，改用 WSL Docker 容器网络读取当前 `prisma/` 目录执行 `/app/node_modules/.bin/prisma migrate status` 通过，显示 14 个迁移且 `Database schema is up to date!`；`npm.cmd run e2e` 已执行但环境阻塞，健康检查通过，访问 `/blog` 与登录用例因 Prisma `ECONNREFUSED` 失败，`Test-NetConnection 127.0.0.1:5432`、WSL IP 与 Docker bridge IP 均为 `TcpTestSucceeded: False`，而容器内 `pg_isready` 和 `select 1` 均通过。
